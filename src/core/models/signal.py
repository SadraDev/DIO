...
class Signal:
    ...
    def __init__(self, action: SignalAction, entry_price: float, stop_loss: float, 
        ...
        # Outcome tracking
        self.outcome: Optional[SignalOutcome] = None
    ...
    def evaluate_signal(self, risk_free: bool = True, max_fetch_attempts: int = 5) -> Optional['Bar']:
        """
        ...
        # Calculate position size using Budget's risk_amount if not set
        if self.entry_lot is None:
            self.entry_lot = budget.lots_from_diff( 
                self.symbol, abs(self.initial_entry_price - self.initial_stop_loss))
 
        # Update pips calculations if missing using Budget
        if self.stop_loss_pips is None:
            self.stop_loss_pips = budget.pips_from_diff(abs(self.initial_entry_price - self.initial_stop_loss))
        
        if self.take_profit_pips is None:
            self.take_profit_pips = budget.pips_from_diff(abs(self.initial_take_profit - self.initial_entry_price))
        ...
        while fetch_attempts <= max_fetch_attempts:
            ...
            for bar in evaluation_bars:
                ...
                if self.action == SignalAction.SELL:
                    ...
                    if bar.low <= self.take_profit:
                        ...
                        log_signal_event("signal_win", self.symbol, self.action.value,
                                        gain=actual_gain, exit_price=self.take_profit,
                                        pips=budget.pips_from_diff(abs(self.initial_entry_price - self.take_profit)),
                                        lot_size=self.entry_lot, bars_evaluated=len(evaluation_bars),
                                        fetch_attempts=fetch_attempts)
                    
                    # Stop Loss hit (price going up)
                    elif bar.high >= self.stop_loss:
                    ...
                    if bar.high >= self.take_profit:
                        ...
                        log_signal_event("signal_win", self.symbol, self.action.value,
                                        gain=actual_gain, exit_price=self.take_profit,
                                        pips=budget.pips_from_diff(abs(self.take_profit - self.initial_entry_price)),
                                        lot_size=self.entry_lot, bars_evaluated=len(evaluation_bars),
                                        fetch_attempts=fetch_attempts)
                ...
                if risk_free and bar is not None:
                    if self.action == SignalAction.SELL:
                        ...
                        if abs(bar.low - self.initial_take_profit) <= 0.2 * risk_amount:
                        ...
                        # Near TP profit locking
                        if abs(bar.high - self.initial_take_profit) <= 0.2 * risk_amount:
    ...
    def risk_free(self, bars_till_now: List['Bar'], buffer_pips: float = 3.0) -> bool:
        """
        LEGACY METHOD: Adjust stop loss for risk-free exit
        NOTE: This is now integrated into evaluate_with_bars() for single-loop processing
        """
        ...
        try:
            ...
            pip_size = settings.get('market.standard_pip_size', 0.0001)
        except:
            pip_size = 0.0001
        ...
        for bar in bars_till_now:
            if self.action == SignalAction.SELL:
                if bar.low <= self.initial_entry_price - risk_amount:
                    if self.stop_loss >= self.initial_stop_loss:
                        new_sl = self.initial_stop_loss - risk_amount / 2
                        self.adjust_stop_loss(new_sl, "1R_favorable_movement")
                        adjusted = True
                
                if bar.low <= self.initial_entry_price - 2 * risk_amount:
                    if self.initial_entry_price < self.stop_loss <= self.initial_stop_loss:
                        self.adjust_stop_loss(self.initial_entry_price + buffer, "2R_breakeven")
                        adjusted = True
                        
            elif self.action == SignalAction.BUY:
                if bar.high >= self.initial_entry_price + risk_amount:
                    if self.stop_loss <= self.initial_stop_loss:
                        new_sl = self.initial_stop_loss + risk_amount / 2
                        self.adjust_stop_loss(new_sl, "1R_favorable_movement")
                        adjusted = True
                
                if bar.high >= self.initial_entry_price + 2 * risk_amount:
                    if self.initial_entry_price > self.stop_loss >= self.initial_stop_loss:
                        self.adjust_stop_loss(self.initial_entry_price - buffer, "2R_breakeven")
                        adjusted = True
